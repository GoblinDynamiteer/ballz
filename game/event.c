...
bool event_process_events(ballsGame *game) {
    ...
    if ((mouseState & SDL_BUTTON(SDL_BUTTON_LEFT)) && game->mode == NEO &&
...
double event_calculate_distance(int x1, int x2, int y1, int y2) {
    double calc = sqrt(pow((float)x1 - (float)x2, 2) + pow((float)y1 - (float)y2, 2));
...
void event_cycle_game_mode(ballsGame *game, bool cycle_direction) {
    if (cycle_direction) {
        if (game->mode == MAX_GAME_TYPES - 1) {
            ...
            game->mode = (short)(game->mode + 1);
        }
    } else {
        if (game->mode == NORMAL) {
            game->mode = (short)(MAX_GAME_TYPES - 1);
        } else {
            game->mode = (short)(game->mode - 1);
        }
    }
